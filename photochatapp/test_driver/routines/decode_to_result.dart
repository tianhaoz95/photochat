...
Future<void> checkDecodeToResult(FlutterDriver driver) async {
  String decodeToResultTestLoggingNamespace = 'decode_to_result_integration_test';
  ...
  await Future.delayed(const Duration(seconds: 1));
  logger.info('try to tap on send to decode result button', name: decodeToResultTestLoggingNamespace);
  await driver.tap(find.byValueKey('decode_screen_decode_btn'));
  await Future.delayed(const Duration(seconds: 1));
  logger.info('should have landed on the decode result screen', name: decodeToResultTestLoggingNamespace);
  await driver.tap(find.pageBack());
  await Future.delayed(const Duration(seconds: 1));
  logger.info('should have been back from decode screen', name: decodeToResultTestLoggingNamespace);
}
